...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt macutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(macutil.Amount(5 * macutil.SatoshiPerBitcoin))
	...
	txid = genSpend(macutil.Amount(500 * macutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(macutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*macutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, macutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := macutil.Amount(250 * macutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = macutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := macutil.Amount(50 * macutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := macutil.Amount(numMatureOutputs * 50 * macutil.SatoshiPerBitcoin)
