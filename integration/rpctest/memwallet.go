...
type memWallet struct {
	...
	addrs map[uint32]macutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]macutil.Address)
...
// SetRPCClient saves the passed rpc connection to macd as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*macutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          macutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (macutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []macutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt macutil.Amount, feeRate macutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := macutil.Amount(txSize * int(feeRate))
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate macutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt macutil.Amount
	for _, output := range outputs {
		outputAmt += macutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() macutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance macutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (macutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := macutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
