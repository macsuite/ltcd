...
import (
	...
	rpc "github.com/macsuite/macd/rpcclient"
	"github.com/macsuite/macutil"
)

// nodeConfig contains all the args, and data required to launch a macd process
...
// arguments returns an array of arguments that be used to launch the macd
// process.
...
// command returns the exec.Cmd which will be used to start the macd process.
...
// to the macd process that is launched via Start().
...
// as the base for the log and data directories for macd.
...
// start creates a new macd process, and writes its pid in a file reserved for
...
// stop interrupts the running macd process process, and waits until it exits
...
// shutdown terminates the running macd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := macutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
