...
func ExamplePayToAddrScript() {
	// Parse the address to send the coins to into a macutil.Address
	// which is useful to ensure the accuracy of the address and determine
	// the address type.  It is also required for the upcoming call to
	// PayToAddrScript.
	addressStr := "12gpXQVcCL2qhTNQgyLVdCFG2Qs2px98nV"
	address, err := macutil.DecodeAddress(addressStr, &chaincfg.MainNetParams)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleSignTxOutput() {
	...
	pubKeyHash := macutil.Hash160(pubKey.SerializeCompressed())
	addr, err := macutil.NewAddressPubKeyHash(pubKeyHash,
	...
	lookupKey := func(a macutil.Address) (*btcec.PrivateKey, bool, error) {
