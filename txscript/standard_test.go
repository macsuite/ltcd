...
// newAddressPubKey returns a new macutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) macutil.Address {
	addr, err := macutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new macutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) macutil.Address {
	addr, err := macutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new macutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) macutil.Address {
	addr, err := macutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the macutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the macutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := macutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := macutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := macutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := macutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := macutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*macutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*macutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*macutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*macutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*macutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
