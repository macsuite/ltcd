...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr macutil.Address) (*btcec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr macutil.Address) ([]byte, error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address, err := macutil.NewAddressPubKeyHash(
				macutil.Hash160(pk), &chaincfg.TestNet4Params)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*btcec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := macutil.NewAddressPubKey(pk,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			scriptAddr, err := macutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk1 := (*btcec.PublicKey)(&key1.PublicKey).
				SerializeCompressed()
			address1, err := macutil.NewAddressPubKey(pk1,
				&chaincfg.TestNet4Params)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			pk2 := (*btcec.PublicKey)(&key2.PublicKey).
				SerializeCompressed()
			address2, err := macutil.NewAddressPubKey(pk2,
				&chaincfg.TestNet4Params)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*macutil.AddressPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
