...
Bitcoin RPC server that uses a macd/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with macd (https://github.com/macsuite/macd),
btcwallet (https://github.com/btcsuite/btcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, macd and
...
macd or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by macd and
...
All notifications provided by macd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by macd (and
...
Also, it is important to realize that macd intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the macd RPC server directly, only the RPCs which are related to
