...
const (
	// svcName is the name of macd service.
	svcName = "macdsvc"
...
// logServiceStartOfDay logs information about macd when the main server has
...
// macdService houses the main service handler which handles all service
// updates and launching macdMain.
...
// long-running macdMain (which is the real meat of macd), handles service
// change requests, and notifies the service control manager of changes.
func (s *macdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start macdMain in a separate goroutine so the service can start
...
// installService attempts to install the macd service.  Typically this should
...
// removeService attempts to uninstall the macd service.  Typically this should
...
// startService attempts to start the macd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &macdService{})
	if err != nil {
