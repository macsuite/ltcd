...
import (
	...
	"github.com/macsuite/macd/addrmgr"
	...
	"github.com/macsuite/macd/mining/cpuminer"
	"github.com/macsuite/macd/peer"
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a macutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := macutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a macutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := macutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > macutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, macutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *macutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			macdLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new macd server configured to listen on addr for the
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, macdLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *macutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
