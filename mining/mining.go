...
const (
	...
	MinHighPriority = macutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr macutil.Address) (*macutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *macutil.Tx, height int32) error {
...
func logSkippedDeps(tx *macutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress macutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*macutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := macutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := macutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A macutil.Block should use saved in the state to avoid
