...
const (
	...
	DefaultMinRelayTxFee = macutil.Amount(100000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee macutil.Amount) int64 {
	...
	if minFee < 0 || minFee > macutil.MaxSatoshi {
		minFee = macutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *macutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee macutil.Amount) bool {
...
func checkTransactionStandard(tx *macutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee macutil.Amount,
...
func GetTxVirtualSize(tx *macutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
