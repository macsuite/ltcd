...
5. [How do I use bootstrap.dat with macd?](#Importing)
...
### 5. How do I use bootstrap.dat with macd?

macd comes with a separate utility named `addblock` which can be used to import
...
1. Stop macd if it is already running.  This is required since addblock needs to
   access the database used by macd and it will be locked if macd is using it.
