...
macd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, macd supports a hybrid,
...
Configuring macd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the macd command
line or in the macd configuration file.  Typically the Tor proxy address will be
...
By default, macd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since macd will act as the server.
...
```bash
$ ./macd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
macd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the macd command line or in the macd
...
```bash
$ ./macd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
macd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./macd --proxy=127.0.0.1:9050 --torisolation
```
