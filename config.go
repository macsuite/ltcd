...
import (
	...
	"github.com/macsuite/macd/connmgr"
	"github.com/macsuite/macd/database"
	_ "github.com/macsuite/macd/database/ffldb"
	"github.com/macsuite/macd/mempool"
...
const (
	defaultConfigFilename        = "macd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "macd.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-macd.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = macutil.AppDataDir("macd", false)
...
// config defines the configuration options for macd.
//
...
type config struct {
	...
	Listeners            []string      `long:"listen" description:"Add an interface/port to listen for connections (default all interfaces port: 40333, testnet: 140333)"`
	...
	Generate             bool          `long:"generate" description:"Generate (mine) machinecoins using the CPU"`
...
//
// The above results in macd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		macdLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = macutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]macutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := macutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		macdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-macd.conf to the given destination path,
...
// macdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func macdDial(addr net.Addr) (net.Conn, error) {
...
// macdLookup resolves the IP of the given host using the correct DNS lookup
...
func macdLookup(host string) ([]net.IP, error) {
