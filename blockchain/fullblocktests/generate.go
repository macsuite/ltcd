...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: macutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := macutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*macutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, macutil.NewTx(tx))
	}
...
func additionalCoinbase(amount macutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee macutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee macutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee macutil.Amount) *wire.MsgTx {
