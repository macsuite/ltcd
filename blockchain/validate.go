...
const (
	...
	baseSubsidy = 50 * macutil.SatoshiPerBitcoin
)
...
func IsCoinBase(tx *macutil.Tx) bool {
...
func IsFinalizedTransaction(tx *macutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *macutil.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if totalSatoshi > macutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// txscript.
func CountSigOps(tx *macutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *macutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(block *macutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *macutil.Block, powLimit *big.Int, timeSource MedianTimeSource) error {
...
func ExtractCoinbaseHeight(coinbaseTx *macutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *macutil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *macutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
func (b *BlockChain) checkBIP0030(node *blockNode, block *macutil.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *macutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", macutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > macutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				macutil.Amount(originTxSatoshi),
				macutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > macutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *macutil.Block, view *UtxoViewpoint, stxos *[]spentTxOut) error {
...
func (b *BlockChain) CheckConnectBlock(block *macutil.Block) error {
	b.chainLock.Lock()
